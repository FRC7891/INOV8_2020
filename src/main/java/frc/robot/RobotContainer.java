...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    m_hooksubsystem.setDefaultCommand(
      new RunCommand(() -> m_hooksubsystem.level(m_operator.getRawAxis(OIConstants.RightTrigger), m_operator.getRawAxis(OIConstants.LeftTrigger)), m_hooksubsystem));
...
*/
    m_elevatorsubsystem.setDefaultCommand(
      new RunCommand(() -> m_elevatorsubsystem.raise(m_operator.getRawAxis(OIConstants.RightStickY)), m_elevatorsubsystem)
    );
    m_IntakeSubsystem.setDefaultCommand(
      new RunCommand(() -> m_IntakeSubsystem.suck(m_operator.getRawAxis(OIConstants.LeftStickY)*-0.70), m_IntakeSubsystem)
    );
      m_hoppersubsystem.setDefaultCommand(
          new RunCommand(() -> m_hoppersubsystem.spin(m_operator.getRawAxis(OIConstants.LeftStickY) * 0.90),
              m_hoppersubsystem));


      //    m_hoppersubsystem.setDefaultCommand(
//      new RunCommand(() -> m_hoppersubsystem.TransportForward(m_operator.getRawAxis(OIConstants.LeftStickY)),
//            m_IntakeSubsystem)
//    );
    m_hooksubsystem
        .setDefaultCommand(new RunCommand(() -> m_hooksubsystem.level(m_opperator.getRawAxis(OIConstants.RightTrigger),
            m_opperator.getRawAxis(OIConstants.LeftTrigger)), m_hooksubsystem));
    ...
    m_elevatorsubsystem.setDefaultCommand(new RunCommand(
        () -> m_elevatorsubsystem.raise(m_opperator.getRawAxis(OIConstants.RightStickY)), m_elevatorsubsystem));
    m_IntakeSubsystem.setDefaultCommand(new RunCommand(
        () -> m_IntakeSubsystem.suck(m_opperator.getRawAxis(OIConstants.LeftStickY) * -0.70), m_IntakeSubsystem));
    m_hoppersubsystem.setDefaultCommand(new RunCommand(
        () -> m_hoppersubsystem.spin(m_opperator.getRawAxis(OIConstants.LeftStickY) * 0.90), m_hoppersubsystem));
...
//When the Y button on the Xbox controller is pressed, a new command will be run which moves the hopper
...
//Okay I need to update this text
*/
...
//I may make it so that it can only do it after the shooter speed is reached, along with an overrride
//mode just in case
...
*/
    ...
    // new JoystickButton(m_opperator, Button.kStickLeft.value).whenPressed(
    // new RunCommand(() -> m_hoppersubsystem.transportForward(),
    // m_hoppersubsystem).alongWith(new RunCommand(
    // () -> m_IntakeSubsystem.suck(m_opperator.getRawAxis(OIConstants.LeftStickY)),
    // m_IntakeSubsystem)));

    // This code is for trickling ballls into low goals while aligned to the
    // goal(need testing for values)
    new JoystickButton(m_opperator, Button.kA.value).whenPressed(() -> m_ShooterSubsystem.ballMovingPID(1000, 1000))
        .whenReleased(() -> m_ShooterSubsystem.ballMovingPID(0, 0));
    // This code if for shooting balls for high goal from fixed position(need
    // testing for values)
    new JoystickButton(m_opperator, Button.kY.value).whenPressed(() -> m_ShooterSubsystem.ballMovingPID(0, 0))
        .whenReleased(() -> m_ShooterSubsystem.ballMovingPID(0, 0));
    // This code is for un-jamming the hopper
    new JoystickButton(m_opperator, Button.kB.value).whenPressed(() -> m_ShooterSubsystem.ballMovingPID(0, 0))
        .whenReleased(() -> m_ShooterSubsystem.ballMovingPID(0, 0));

    // this is for the quick turn function
    new JoystickButton(m_controller, Button.kA.value)
        .whileHeld(() -> new TurnyBoiTheSequal(m_drivetrainsubsystem, m_controller));
    new JoystickButton(m_controller, Button.kStart.value).whenPressed(() -> m_drivetrainsubsystem.zeroSensors(),
        m_drivetrainsubsystem);

    /*
     * //This code is for trickling ballls into low goals while aligned to the
     * goal(need testing for values) new JoystickButton(m_opperator,
     * Button.kA.value) .whenPressed(() ->
     * m_ShooterSubsystem.ballMovingFunction(0.3,0.3)) .whenReleased(() ->
     * m_ShooterSubsystem.ballMovingFunction(0,0)); //This code if for shooting
     * balls for high goal from fixed position(need testing for values) new
     * JoystickButton(m_opperator, Button.kY.value) .whenPressed(() ->
     ...
     * slider shooter new JoystickButton(m_opperator,Button.kX.value)
     * .whenPressed(() -> m_ShooterSubsystem.sliderValueFunction()) .whenReleased(()
     * -> m_ShooterSubsystem.ballMovingFunction(0, 0));
     *
     * //This code is for passing balls from loading zone to trench(need testing for
     * values) new JoystickButton(m_opperator, Button.kY.value) .whenPressed(() ->
     * m_ShooterSubsystem.ballMovingFunction(.4,.5)) .whenReleased(() ->
     * m_ShooterSubsystem.ballMovingFunction(0, 0));
     *
     * //This code is for un-jamming the hopper new JoystickButton(m_opperator,
     ...
     *
     *
     * new JoystickButton(m_opperator, Button.kStickLeft.value) .whenPressed(new
